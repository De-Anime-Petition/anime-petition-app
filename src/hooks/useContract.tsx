...
export default function useContract() {
  ...
  const getNFTTotalMinted = useCallback(
    (nftId: NFTId): Promise<bigint> =>
      readContract('totalMinted', [BigInt(nftId)]),
    [readContract],
  )
  ...
  const getNFTMaxSupply = useCallback(
    (nftId: NFTId): Promise<bigint> =>
      readContract('maxSupply', [BigInt(nftId)]),
    [readContract],
  )

  const getNFTMintPrice = useCallback(
    (nftId: NFTId): Promise<bigint> =>
      readContract('mintPrice', [BigInt(nftId)]),
    [readContract],
  )

  const getNFTUri = useCallback(
    (nftId: NFTId): Promise<string> =>
      readContract('animeRoleURIs', [BigInt(nftId)]),
    [readContract],
  )

  const getNFTMetadata = useCallback(
    async (uri: string): Promise<NFTMetadata> => {
      try {
  ...
  const getAllNFTs = useCallback(async (): Promise<NFTData[]> => {
    const nfts = await Promise.all(
      NFT_IDS.map(async (nftId) => {
        const [totalMinted, maxSupply, mintPrice, uri, userMinted] = await Promise.all([
    ...
    return nfts
  }, [getNFTTotalMinted, getNFTMaxSupply, getNFTMintPrice, getNFTUri, getNFTBalance, getNFTMetadata])

  const getUserMintedNFTs = useCallback(async (): Promise<
    Array<Omit<UserNFT, 'metadata'>>
  > => {
